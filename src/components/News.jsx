...
const News = ({ simplified }) => {
  ...
  const [moreNews, setMoreNews] = useState(12);


  const { data: cryptoNews } = useGetCryptoNewsQuery(
    { newsCategory,
      count: simplified ? 6 : moreNews
    });
    
  if(!cryptoNews?.value) return <Loader />;

  const loadMoreNews = () => {
    setMoreNews(moreNews + 12)
  }
  ...
  return (
    <Row gutter={[24, 24]}>
      ...
      {cryptoNews.value.map((news, i) => (
        ...
        {moreNews <= 35 && newsCategory == 'Cryptocurrency' && (
          <div onClick={loadMoreNews} className='justForSingleButton'>
            <RightOutlined />
              Load More News
            <LeftOutlined />
          </div>
